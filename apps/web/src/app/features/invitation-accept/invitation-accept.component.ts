...
import { InvitationAcceptApiService } from './invitation-accept-api.service';
...
export class InvitationAcceptComponent {
  ...
  private readonly api = inject(InvitationAcceptApiService);
  ...
  readonly step = signal<'loading' | 'expired' | 'invalid' | 'form' | 'submitting' | 'success' | 'error'>('loading');
  ...
  private async validateLink(): Promise<void> {
    try {
      const result = await this.api.validateInvitationLink(this.token, this.signature);
      ...
      if (error instanceof HttpErrorResponse) {
        if (error.error?.detail) {
          ...
          this.errorMessage.set('This invitation is no longer valid. Ask your Director to send a new one.');
        }
      } else {
