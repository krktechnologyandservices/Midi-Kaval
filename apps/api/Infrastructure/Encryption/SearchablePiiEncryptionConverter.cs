...
/// <summary>
/// AES-256-GCM value converter using deterministic nonces derived via HMAC-SHA256.
/// Same plaintext + same key always produces the same ciphertext, enabling exact-match
/// WHERE clauses on encrypted fields (e.g., beneficiary_name searches).
/// Encrypts with the active key; decrypts using the key version embedded in the ciphertext.
/// </summary>
/// <remarks>
/// Security: The nonce is HMAC-SHA256(plaintext) truncated to 12 bytes. Since the nonce
/// is a deterministic function of the plaintext, two encryptions of the same plaintext
/// under the same key produce identical ciphertext. This leaks equality patterns but is
/// acceptable for this internal case management system.
/// 
/// IMPORTANT: Does NOT support ILike / LIKE '%search%' queries — only exact (=) matching.
/// </remarks>
public sealed class SearchablePiiEncryptionConverter : ValueConverter<string?, byte[]?>
{
    private const int NonceSize = 12;
    private const int TagSize = 16;

    public SearchablePiiEncryptionConverter()
        : base(
            v => Encrypt(v),
            v => Decrypt(v))
    { }
    ...
    private static byte[]? Encrypt(string? plaintext)
    {
        ...
        // Derive deterministic nonce: HMAC-SHA256(key, plaintext) → first 12 bytes
        var nonce = DeriveDeterministicNonce(key, plaintextBytes);
    ...
    private static byte[] DeriveDeterministicNonce(byte[] key, byte[] plaintext)
    {
