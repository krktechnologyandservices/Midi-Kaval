...
public sealed class CaseStage5DataController(
    CaseStage5DataService stage5DataService) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = Policies.CoordinatorOrAbove)]
    [ProducesResponseType(typeof(Stage5ReintegrationDto), StatusCodes.Status200OK)]
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var dto = await stage5DataService.GetAsync(caseId, cancellationToken);
            return Ok(dto);
        }
        catch (CaseNotFoundException)
        {
            return NotFoundProblem("Case not found or not in Stage 5.");
        }
    ...
    public async Task<IActionResult> Upsert(
        Guid caseId,
        [FromBody] UpsertStage5ReintegrationRequest request,
        CancellationToken cancellationToken)
    {
        if (request is null)
        {
        ...
        try
        {
            var dto = await stage5DataService.UpsertAsync(caseId, request, cancellationToken);
            return Ok(dto);
        }
        catch (CaseNotFoundException)
        {
